...
15. Write a program that encrypts a message using a Caesar cipher. The user
     will enter the message to be encrypted and the shift amount (the number
     of positions by which the letters should be shifted). You may assume the
     message does not exceed 80 characters. Characters other than letters
     should be left unchanged. Letter case should stay the same. To handle the
     wrap around problem, use the expression ((ch - 'A') + n) % 26 + 'A' to
     calculate the encrypted version of an uppercase letter, where ch stores
     the letter and n is the shift amount.

#include <stdio.h>

#define UPPERWRAP ((message[i] - 'A') + shift) % 26 + 'A'
#define LOWERWRAP ((message[i] - 'a') + shift) % 26 + 'a'
...
16. Write a program that tests whether two words are anagrams. Write a loop
     that reads the first word, using an array of 26 integers to keep track of
     how many times each letter has been seen. Use another loop to read the
     second word, except this time decrementing the corresponding array element
     as each letter is read. Both loops should ignore any character that is not
     a letter, and both should treat upper-case letters in the same way as
     lower-case letters. After the second word has been read, use a third loop
     to check whether all the elements in the array are zero.
...
{
    short i;
    char ch, first_word[26] = {0}, second_word[26] = {0};
...
17. Write a program that prints an n X n magic square (a square arrangement of
     numbers in which the sums of the rows, columns, and diagonals are all the
     same. The user will specify the value of n. Store the magic square in a
...
int main(void)
{
    signed int y = 0, x = 0, i = 1, n = 1;
    signed int magic[n][n];
